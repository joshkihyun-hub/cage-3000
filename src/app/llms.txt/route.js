...
import { PROJECT_GROUPS } from '@/shared/constants/project1-images';
...
> 조각적인 실루엣의 의상과 헤드웨어를 한 점씩 만들며, 커스텀 메이드 제작도 가능합니다.
> A designer fashion brand from Seoul, led by Kihyun Kim — garments and headwear,
> made to order, each piece by hand. Custom-made pieces on request.
...
- 커스텀 메이드: 아티스트·작품을 위한 의뢰 제작 가능 (custom made on request) — contact@cage3000.com
...
## Projects
${PROJECT_GROUPS.map((group) => `### ${group.label_ko} (${group.label})\n${group.items.map((project) => `- [${project.title}](${SITE_URL}/projects/${project.slug}): ${project.description_ko}`).join('\n')}`).join('\n\n')}
`;
